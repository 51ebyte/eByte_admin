	...
	<Row>
		<Form ref="slideForm" :model="form.data" :rules="form.rule" :label-width="100">
			...
			<FormItem :label-width="0">
				<Upload ref="upload" type="drag" accept="image/png,image/jpeg,image/gif" :show-upload-list="false" 
				:action="$upload.image" :format="['jpg','jpeg','png']" name="file" :on-success="uploadSuccess">
			...
			<FormItem :label-width="0" v-if="form.data.image">
				<div class="upload-preview">
					<img :src="$assets.url+form.data.image" />
				</div>
			</FormItem>
			</Col>
		</Form>
