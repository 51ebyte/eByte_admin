<template>
	<div>
		<Upload v-if="!audio" :action="$upload.audio" :show-upload-list="false"
			...
			:on-success="uploadAudioSuccess" :on-error="uploadAudioError" 
			:on-progress="uploadAudioProgress" :on-format-error="uploadAudioFormatError">
	...
	export default{
		...
		mounted(){
			this.$nextTick(function(){
				if(this.audio!=''){
					this.music.url=this.$assets.url+this.audio;
		...
		methods: {
			...
			uploadAudioSuccess(response){
				if(response.code==200){
					this.music.url=this.assets.url+response.data.link;
